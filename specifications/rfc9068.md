...
description:  This specification defines a profile for issuing OAuth 2.0 access tokens in JSON Web Token (JWT) format. Authorization servers and resource servers from different vendors can leverage this profile to issue and consume access tokens in an interoperable manner.
generator: xml2rfc 3.10.0
keyword:
- OAuth
- Resource
- Access token
...
Information about the current status of this document, any errata, and how to provide feedback on it may be obtained at <https://www.rfc-editor.org/info/rfc9068>.[¶](#section-boilerplate.1-3)
...
-   [2](#section-2).  [JWT Access Token Header and Data Structure](#name-jwt-access-token-header-and)

    -   [2.1](#section-2.1).  [Header](#name-header)

    -   [2.2](#section-2.2).  [Data Structure](#name-data-structure)

        -   [2.2.1](#section-2.2.1).  [Authentication Information Claims](#name-authentication-information-)

        -   [2.2.2](#section-2.2.2).  [Identity Claims](#name-identity-claims)

        -   [2.2.3](#section-2.2.3).  [Authorization Claims](#name-authorization-claims)

            -   [2.2.3.1](#section-2.2.3.1).  [Claims for Authorization Outside of Delegation Scenarios](#name-claims-for-authorization-ou)

-   [3](#section-3).  [Requesting a JWT Access Token](#name-requesting-a-jwt-access-tok)

-   [4](#section-4).  [Validating JWT Access Tokens](#name-validating-jwt-access-token)

-   [5](#section-5).  [Security Considerations](#name-security-considerations)

-   [6](#section-6).  [Privacy Considerations](#name-privacy-considerations)

-   [7](#section-7).  [IANA Considerations](#name-iana-considerations)

    -   [7.1](#section-7.1).  [Media Type Registration](#name-media-type-registration)

        -   [7.1.1](#section-7.1.1).  [Registry Content](#name-registry-content)

    -   [7.2](#section-7.2).  [Claims Registration](#name-claims-registration)

        -   [7.2.1](#section-7.2.1).  [Registry Content](#name-registry-content-2)

            -   [7.2.1.1](#section-7.2.1.1).  [Roles](#name-roles)

            -   [7.2.1.2](#section-7.2.1.2).  [Groups](#name-groups)

            -   [7.2.1.3](#section-7.2.1.3).  [Entitlements](#name-entitlements)
...
Please note: Although both this document and \[[RFC7523](#RFC7523)\] use JSON Web Tokens in the context of the OAuth2 framework, the two specifications differ in both intent and mechanics. Whereas \[[RFC7523](#RFC7523)\] defines how a JWT Bearer Token can be used to request an access token, this document describes how to encode access tokens in JWT format.[¶](#section-1-6)
...
This specification uses the terms \"access token\", \"refresh token\", \"authorization server\", \"resource server\", \"authorization endpoint\", \"authorization request\", \"authorization response\", \"token endpoint\", \"grant type\", \"access token request\", \"access token response\", and \"client\" defined by [The OAuth 2.0 Authorization Framework](#RFC6749) \[[RFC6749](#RFC6749)\].[¶](#section-1.2-2)

## [2.](#section-2) [JWT Access Token Header and Data Structure](#name-jwt-access-token-header-and)

### [2.1.](#section-2.1) [Header](#name-header)

JWT access tokens MUST be signed. Although JWT access tokens can use any signing algorithm, use of asymmetric cryptography is RECOMMENDED as it simplifies the process of acquiring validation information for resource servers (see [Section 4](#JWTATLValidate)). JWT access tokens MUST NOT use \"none\" as the signing algorithm. See [Section 4](#JWTATLValidate) for more details.[¶](#section-2.1-1)

Authorization servers and resource servers conforming to this specification MUST include RS256 (as defined in \[[RFC7518](#RFC7518)\]) among their supported signature algorithms.[¶](#section-2.1-2)

This specification registers the \"application/at+jwt\" media type, which can be used to indicate that the content is a JWT access token. JWT access tokens MUST include this media type in the \"typ\" header parameter to explicitly declare that the JWT represents an access token complying with this profile. Per the definition of \"typ\" in [Section 4.1.9](https://www.rfc-editor.org/rfc/rfc7515#section-4.1.9) of \[[RFC7515](#RFC7515)\], it is RECOMMENDED that the \"application/\" prefix be omitted. Therefore, the \"typ\" value used SHOULD be \"at+jwt\". See the [Security Considerations](#SecurityConsiderations) section for details on the importance of preventing OpenID Connect ID Tokens (as defined by Section 2 of \[[OpenID.Core](#OpenID.Core)\]) from being accepted as access tokens by resource servers implementing this profile.[¶](#section-2.1-3)
...
iss
:   REQUIRED - as defined in [Section 4.1.1](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.1) of \[[RFC7519](#RFC7519)\].[¶](#section-2.2-2.2)
:   

exp
:   REQUIRED - as defined in [Section 4.1.4](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.4) of \[[RFC7519](#RFC7519)\].[¶](#section-2.2-2.4)
:   

aud
:   REQUIRED - as defined in [Section 4.1.3](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.3) of \[[RFC7519](#RFC7519)\]. See [Section 3](#JWTATLRequest) for indications on how an authorization server should determine the value of \"aud\" depending on the request.[¶](#section-2.2-2.6)
:   

sub
:   REQUIRED - as defined in [Section 4.1.2](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.2) of \[[RFC7519](#RFC7519)\]. In cases of access tokens obtained through grants where a resource owner is involved, such as the authorization code grant, the value of \"sub\" SHOULD correspond to the subject identifier of the resource owner. In cases of access tokens obtained through grants where no resource owner is involved, such as the client credentials grant, the value of \"sub\" SHOULD correspond to an identifier the authorization server uses to indicate the client application. See [Section 5](#SecurityConsiderations) for more details on this scenario. Also, see [Section 6](#PrivacyConsiderations) for a discussion about how different choices in assigning \"sub\" values can impact privacy.[¶](#section-2.2-2.8)
:   

client_id
:   REQUIRED - as defined in [Section 4.3](https://www.rfc-editor.org/rfc/rfc8693#section-4.3) of \[[RFC8693](#RFC8693)\].[¶](#section-2.2-2.10)
:   

iat
:   REQUIRED - as defined in [Section 4.1.6](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.6) of \[[RFC7519](#RFC7519)\]. This claim identifies the time at which the JWT access token was issued.[¶](#section-2.2-2.12)
:   

jti
:   REQUIRED - as defined in [Section 4.1.7](https://www.rfc-editor.org/rfc/rfc7519#section-4.1.7) of \[[RFC7519](#RFC7519)\].[¶](#section-2.2-2.14)
:   

#### [2.2.1.](#section-2.2.1) [Authentication Information Claims](#name-authentication-information-)

The claims listed in this section MAY be issued in the context of authorization grants involving the resource owner and reflect the types and strength of authentication in the access token that the authentication server enforced prior to returning the authorization response to the client. Their values are fixed and remain the same across all access tokens that derive from a given authorization response, whether the access token was obtained directly in the response (e.g., via the implicit flow) or after one or more token exchanges (e.g., obtaining a fresh access token using a refresh token or exchanging one access token for another via \[[RFC8693](#RFC8693)\] procedures).[¶](#section-2.2.1-1)

auth_time
:   OPTIONAL - as defined in Section 2 of \[[OpenID.Core](#OpenID.Core)\].[¶](#section-2.2.1-2.2)
:   

acr
:   OPTIONAL - as defined in Section 2 of \[[OpenID.Core](#OpenID.Core)\].[¶](#section-2.2.1-2.4)
:   

amr
:   OPTIONAL - as defined in Section 2 of \[[OpenID.Core](#OpenID.Core)\].[¶](#section-2.2.1-2.6)
:   

#### [2.2.2.](#section-2.2.2) [Identity Claims](#name-identity-claims)

In the context of authorization grants involving the resource owner, commercial authorization servers will often include resource owner attributes directly in access tokens so that resource servers can consume them directly for authorization or other purposes without any further round trips to introspection (\[[RFC7662](#RFC7662)\]) or UserInfo (\[[OpenID.Core](#OpenID.Core)\]) endpoints. This is particularly common in scenarios where the client and the resource server belong to the same entity and are part of the same solution, as is the case for first-party clients invoking their own backend API.[¶](#section-2.2.2-1)
...
Any additional identity attribute whose semantic is well described by an entry in the \"JSON Web Token (JWT)\" IANA registry introduced in \[[RFC7519](#RFC7519)\] SHOULD be encoded using the corresponding claim name, if that attribute is to be included in the JWT access token. Note that the JWT IANA registry includes the claims found in Section 5.1 of \[[OpenID.Core](#OpenID.Core)\].[¶](#section-2.2.2-3)

Authorization servers MAY return arbitrary attributes not defined in any existing specification, as long as the corresponding claim names are collision resistant or the access tokens are meant to be used only within a private subsystem. Please refer to Sections [4.2](https://www.rfc-editor.org/rfc/rfc7519#section-4.2) and [4.3](https://www.rfc-editor.org/rfc/rfc7519#section-4.3) of \[[RFC7519](#RFC7519)\] for details.[¶](#section-2.2.2-4)

Authorization servers including resource owner attributes in JWT access tokens need to exercise care and verify that all privacy requirements are met, as discussed in [Section 6](#PrivacyConsiderations).[¶](#section-2.2.2-5)

#### [2.2.3.](#section-2.2.3) [Authorization Claims](#name-authorization-claims)

If an authorization request includes a scope parameter, the corresponding issued JWT access token SHOULD include a \"scope\" claim as defined in [Section 4.2](https://www.rfc-editor.org/rfc/rfc8693#section-4.2) of \[[RFC8693](#RFC8693)\].[¶](#section-2.2.3-1)

All the individual scope strings in the \"scope\" claim MUST have meaning for the resources indicated in the \"aud\" claim. See [Section 5](#SecurityConsiderations) for more considerations about the relationship between scope strings and resources indicated by the \"aud\" claim.[¶](#section-2.2.3-2)

##### [2.2.3.1.](#section-2.2.3.1) [Claims for Authorization Outside of Delegation Scenarios](#name-claims-for-authorization-ou)

Many authorization servers embed authorization attributes that go beyond the delegated scenarios described by \[[RFC7519](#RFC7519)\] in the access tokens they issue. Typical examples include resource owner memberships in roles and groups that are relevant to the resource being accessed, entitlements assigned to the resource owner for the targeted resource that the authorization server knows about, and so on.[¶](#section-2.2.3.1-1)

An authorization server wanting to include such attributes in a JWT access token SHOULD use the \"groups\", \"roles\", and \"entitlements\" attributes of the \"User\" resource schema defined by [Section 4.1.2](https://www.rfc-editor.org/rfc/rfc7643#section-4.1.2) of \[[RFC7643](#RFC7643)\]) as claim types.[¶](#section-2.2.3.1-2)

Authorization servers SHOULD encode the corresponding claim values according to the guidance defined in \[[RFC7643](#RFC7643)\]. In particular, a non-normative example of a \"groups\" attribute can be found in [Section 8.2](https://www.rfc-editor.org/rfc/rfc7643#section-8.2) of \[[RFC7643](#RFC7643)\]. No specific vocabulary is provided for \"roles\" and \"entitlements\".[¶](#section-2.2.3.1-3)

[Section 7.2.1](#IANAClaimsContent) of this document provides entries for registering \"groups\", \"roles\", and \"entitlements\" attributes from \[[RFC7643](#RFC7643)\] as claim types to be used in this profile.[¶](#section-2.2.3.1-4)

## [3.](#section-3) [Requesting a JWT Access Token](#name-requesting-a-jwt-access-tok)

An authorization server can issue a JWT access token in response to any authorization grant defined by \[[RFC6749](#RFC6749)\] and subsequent extensions meant to result in an access token.[¶](#section-3-1)

If the request includes a \"resource\" parameter (as defined in \[[RFC8707](#RFC8707)\]), the resulting JWT access token \"aud\" claim SHOULD have the same value as the \"resource\" parameter in the request.[¶](#section-3-2)
...
[Figure 1](#figure-1): [Authorization Request with Resource and Scope Parameters](#name-authorization-request-with-)
...
[Figure 2](#figure-2): [The Header and JWT Claims Set of a JWT Access Token](#name-the-header-and-jwt-claims-s)

The authorization server MUST NOT issue a JWT access token if the authorization granted by the token would be ambiguous. See [Section 5](#SecurityConsiderations) for more details about common cases that might lead to ambiguity and strategies an authorization server can enact to prevent them.[¶](#section-3-7)

If the request does not include a \"resource\" parameter, the authorization server MUST use a default resource indicator in the \"aud\" claim. If a \"scope\" parameter is present in the request, the authorization server SHOULD use it to infer the value of the default resource indicator to be used in the \"aud\" claim. The mechanism through which scopes are associated with default resource indicator values is outside the scope of this specification. If the values in the \"scope\" parameter refer to different default resource indicator values, the authorization server SHOULD reject the request with \"invalid_scope\" as described in [Section 4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749#section-4.1.2.1) of \[[RFC6749](#RFC6749)\].[¶](#section-3-8)
...
-   The resource server MUST validate the signature of all incoming JWT access tokens according to \[[RFC7515](#RFC7515)\] using the algorithm specified in the JWT \"alg\" Header Parameter. The resource server MUST reject any JWT in which the value of \"alg\" is \"none\". The resource server MUST use the keys provided by the authorization server.[¶](#section-4-5.5)
...
This section registers \"application/at+jwt\", a new media type \[[RFC2046](#RFC2046)\] in the \"Media Types\" registry \[[IANA.MediaTypes](#IANA.MediaTypes)\] in the manner described in \[[RFC6838](#RFC6838)\]. It can be used to indicate that the content is an access token encoded in JWT format.[¶](#section-7.1.1-1)
...
[Section 2.2.3.1](#NDAuthorizationClaims) of this specification refers to the attributes \"roles\", \"groups\", \"entitlements\" defined in \[[RFC7643](#RFC7643)\] to express authorization information in JWT access tokens. This section registers those attributes as claims in the \"JSON Web Token (JWT)\" IANA registry introduced in \[[RFC7519](#RFC7519)\].[¶](#section-7.2-1)
...
:   [Section 4.1.2](https://www.rfc-editor.org/rfc/rfc7643#section-4.1.2) of \[[RFC7643](#RFC7643)\] and [Section 2.2.3.1](#NDAuthorizationClaims) of RFC 9068[¶](#section-7.2.1.1-1.8)
:   
...
Specification Document(s):
:   [Section 4.1.2](https://www.rfc-editor.org/rfc/rfc7643#section-4.1.2) of \[[RFC7643](#RFC7643)\] and [Section 2.2.3.1](#NDAuthorizationClaims) of RFC 9068[¶](#section-7.2.1.2-1.8)
:   
...
Specification Document(s):
:   [Section 4.1.2](https://www.rfc-editor.org/rfc/rfc7643#section-4.1.2) of \[[RFC7643](#RFC7643)\] and [Section 2.2.3.1](#NDAuthorizationClaims) of RFC 9068[¶](#section-7.2.1.3-1.8)
:   
...
John Bradley, Brian Campbell, Vladimir Dzhuvinov, Torsten Lodderstedt, Nat Sakimura, Hannes Tschofenig, and everyone who actively participated in the unconference discussions.[¶](#appendix-A-2)
